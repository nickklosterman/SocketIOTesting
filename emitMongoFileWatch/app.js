...
mongoClient.connect('mongodb://127.0.0.1:27017/bc',function(err,database){
    if (err) {throw err;}
    db = database;
    //deals will be the name of the table/collection
...
function getLastRecordForSite(site) {
	    ...
	    var siteTimer = site+'timer'; //data[0].site+'timer';
	    console.log(siteTimer);
...
function getLastRecord() {
	...
	if (typeof data !== 'undefined' && data !== null && data.length > 0 && typeof data[0].site !== 'undefined' ) {
		...
		//I'm having a problem where the computed timeLeft ends up > than the duration, so i'm using the min as a workaround
		//		    timeLeft = data[0].duration - (parseInt((new Date()).getTime()/1000,10) - data[0].utc_load_time )
		timeLeft =Math.min( data[0].duration - (parseInt((new Date()).getTime()/1000,10) - data[0].utc_load_time ),data[0].duration)
		...
		break; 
	    default:
		console.log('getLastRecord--BROKEN:'+JSON.stringify(data));
		break;
	    }
	    ...
	    console.log('fuck, getLastRecord\'s data check failed');
	}
    });//end collection.find
